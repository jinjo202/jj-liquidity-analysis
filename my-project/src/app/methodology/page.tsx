          ...
          <Accordion>
            ...
            <AccordionItem value="8">
              ...
              <AccordionContent>
                ...
                <p className="mt-2">
                  예외가 하나 있다. 화면 맨 위 &apos;코스피&apos; 칸의 현재가만 네이버 금융
                  시세를 1분마다 받아 온다. FREESIS 는 장 마감 후 공표라 장중에는 전일 종가가
                  최신이기 때문이다. 이 값은 표시 전용이고, 아래 모든 분석은 FREESIS 확정치로
                  계산한다 — 그래서 장중에는 위 지수와 아래 분석 기준일이 하루 어긋날 수 있다.
                </p>
              </AccordionContent>
            </AccordionItem>
            ...
            <AccordionItem value="9">
              ...
              <AccordionContent>
                ...
                <p className="font-medium text-foreground">숏커버 사다리</p>
                <p>
                  마진콜 사다리와 순서만 반대다. 신용융자 쪽은 잔고가 늘어난 날의 지수를 그
                  물량의 매수 지수로 보고 쌓은 뒤 지수가 내려가면 무엇이 반대매매 구간에
                  들어가는지를 봤다. 대차잔고도 같은 방식으로 늘어난 날의 지수에 쌓되, 방향만
                  뒤집어 읽는다 — 숏은 지수가 <em>올라야</em> 손실이므로, 지수가 그 구간 위로
                  올라오면 그 물량이 손실 구간에 들어간다.
                </p>
                <p>
                  대표 진입 지수로는 구간 하단을 쓴다. 마진콜 쪽이 구간 상단을 대표 매수가로
                  쓰는 것과 같은 기준으로, 지수가 움직이는 방향에서 가장 먼저 걸리는 쪽을
                  잡는 것이다. 그래서 현재 지수가 걸쳐 있는 구간은 일부만 손실권이어도 전체를
                  &apos;이미 손실권&apos;으로 센다. 합계는 신용융자와 같은 churn 보정을 거쳐
                  사이클 순증(사이클 시작 잔고 → 잔고 고점)에 맞춘다.
                </p>
                <p className="font-medium text-foreground">한계</p>
                <ul className="list-disc space-y-1.5 pl-5">
                  <li>
                    <span className="font-medium text-foreground">
                      숏커버 사다리는 청산 예정량이 아니다.
                    </span>{' '}
                    신용융자에는 담보유지비율(140%)이라는 공표된 강제 청산 규칙이 있어 지수 얼마
                    아래에서 반대매매가 나오는지 계산할 수 있지만, 대차거래에는 그에 해당하는
                    공표 규칙이 없다. 대주 계약의 증거금·마진콜 조건은 당사자 간 계약이라
                    공개되지 않는다. 그래서 이 사다리는 계수 없이 손실권 진입만 세고, 그 값은
                    실제 커버 물량이 아니라 커버 압력의 상한으로 읽어야 한다.
                  </li>
                  <li>
