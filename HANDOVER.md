...
```
[판정][요약] [1 신용잔고][2 공매도·숏커버][3 레버리지ETF][4 다음주수급][5 종목트래킹][6 국가별포지션][전체]
   ↑ sticky 메뉴(top:0). 판정·요약은 앵커 링크, 나머지는 파트 탭.
오늘의 종합 판정 #top-verdict   ← 지표 12개, 매일 재계산 (§35)
핵심 요약 #top-summary
...
PART n
  ├─ .parthead (파트 제목)
  ├─ .secnav  ← 이 파트 안 h2 목차 (자동 생성)
  └─ 본문. PART 1 안에는 [2025–26][2020–21] 하위 탭
```

**메뉴는 화면에 붙어 있어야 한다**(`position:sticky`). 예전엔 스크롤하면 사라져서
파트를 옮기려면 맨 위로 돌아가야 했다. sticky 로 만드는 대신 카드가 아니라 한 줄 칩으로
줄였다 — 세 줄짜리 카드를 붙여 두면 화면을 너무 먹는다. 파트 설명은 `.parthead` 가 맡는다.

주의할 것 세 개:

- `<input class="tabin">` 라디오는 `.tabs` 와 `.pane` **양쪽보다 앞**에 있어야 한다 —
  형제 선택자(`~`)로 둘 다 제어한다. 탭 바만 위로 올리고 라디오는 그 앞에 둔다.
- `.ic-bar`(구간 선택 바)도 sticky 다. `top:41px` 로 메뉴 **아래**에 붙인다. 그래서 좁은 화면에서
  메뉴가 두 줄로 접히면 겹친다 — 접지 말고 `overflow-x:auto` 로 가로로 굴린다(≤700px).
- 앵커 이동 시 sticky 메뉴가 제목을 덮으므로 `scroll-margin-top:60px`.

### 7.2 파트 안쪽 목차는 자동 생성이다

`addPaneIndexes()` 가 **조립이 끝난 HTML** 에서 만든다. h2 스무 개에 손으로 id 를 달지 않는다.
경계는 각 파트 끝의 `<!-- /p-키 -->` 주석으로 잡는다 — **이 주석을 지우면 목차가 사라진다**.
하나도 못 만들면 빌드가 죽는다. h2 가 하나뿐인 파트(PART 4)는 목차를 안 만든다.
...
단위는 제목이 아니라 **축 옆에** 붙는다(`.axu`). 대화형 차트는 정적 SVG 를 걷어내고 다시
그리는데 거기엔 제목줄이 없어서, 예전에는 대화형으로 전환되는 순간 단위가 화면에서
**아예 사라졌다** — 눈금 숫자만 남아 뭘 재는 그림인지 알 수 없었다.

- `splitUnit('대차잔고 (백만주)')` → `{ title: '대차잔고', axis: '백만주' }`
- 단위 표기는 `KNOWN_UNITS` 로 통일한다(`조` 는 `조원` 으로 정규화).
- **단위를 못 읽으면 빌드가 죽는다** — `chartRuntime()` 이 `axis` 빈 차트를 찾아 던진다.
  `만원` 을 목록에 빼먹어 조용히 라벨이 사라진 적이 있어서 넣은 가드다.
- 눈금 숫자는 `tickFmt` 가 **눈금끼리 구분되는 최소 소수점**만 쓴다(`5,632.398` → `5,632`).
- 값이 다섯 자리를 넘으면 단위를 올린다 — 주가는 `원`→`만원`, 개인 순매수는 `억원`→`조원`.

### 7.3 계열 켜기/끄기 체크박스

PART 1 첫 차트("신용융자 — 시장별 잔고와 코스피 지수")가 이 프로젝트 최초의 **토글 가능한**
다계열 차트다(§37, §40). 전체·코스피·코스닥 신용융자(토글 대상, 조원)에 코스피 지수(보조축,
점선, 늘 켜짐)를 곁들인다. `build.mjs` 의 `seriesToggle()` 로 체크박스 마크업을 만들고
`chart-client.js` 의 `hidden` 상태가 `draw()` 그리기 직전에 꺼진 계열을 걷어낸다 — 눈금·선·
툴팁·구간요약이 자동으로 켜진 것만 본다. 다른 다계열 차트에도 같은 함수로 붙일 수 있다.
(예전엔 이 기능이 별도의 "신용융자 시장별 겹쳐보기" 차트로 따로 있었다 — §40 에서 합쳤다.)
