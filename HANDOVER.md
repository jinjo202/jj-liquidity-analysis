...
```
[PART 1][PART 2][PART 3][PART 4][PART 5][ALL]   ← 큰 탭 (맨 위)
오늘의 종합 판정 (탭 밖)   ← 지표 12개, 매일 재계산 (§35)
핵심 요약 (탭 밖)
...
**탭이 맨 위에 있어야 한다.** 결론 아래에 두면 화면 두 번 넘겨야 나와서 파트 이동을 못 찾는다.
단, `<input class="tabin">` 라디오는 `.tabs` 와 `.pane` **양쪽보다 앞**에 있어야 한다 —
형제 선택자(`~`)로 둘 다 제어하기 때문이다. 탭 바만 위로 올리고 라디오는 그 앞에 둔다.
