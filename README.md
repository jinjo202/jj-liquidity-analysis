...
| **PART 4 — 다음 주 수급** | 지수가 어디로 가면 **무엇이 따라 나오나** (시나리오별 강제 매매·사다리 거리·base rate) |
...
- **`index.html`** — 웹 리포트. 데이터와 SVG 차트를 전부 품고 있어 외부 의존성이 없다.
  `file://` 로 열어도 그대로 보인다. 탭 5개(PART 1~4 / 전체), 스크립트 0줄(라디오 + CSS 형제 선택자).
...
**로컬 PC 와 무관하다.** `runs-on: ubuntu-latest` 로 GitHub 이 제공하는 서버에서 돌고,
커밋 작성자는 `github-actions[bot]` 이다. PC 가 꺼져 있어도, 이 저장소를 클론한 PC 가 하나도
없어도 갱신은 계속된다.

알아둘 것 두 가지:

- **스케줄은 정시가 아니다.** GitHub 부하에 따라 수십 분 밀릴 수 있다. 하루 두 번 도는 이유이기도 하다.
- **60일 무활동이면 GitHub 이 스케줄을 자동으로 끈다.** 평일마다 봇 커밋이 쌓이는 동안은
  걸릴 일이 없지만, 오래 비워 두면 `ranOn` 이 멈춘다. 그때는 Actions 탭에서 다시 켜면 된다.
