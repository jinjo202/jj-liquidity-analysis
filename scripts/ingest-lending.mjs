// FREESIS '대차거래추이'(STATSCU0100000140) 다운로드 파일을 data/lending-balance.json 으로 정규화한다.
//
...
// (§8 신용융자 분리와 같은 이유로 grid 값이 null만 나온다) 통계 화면에서 내려받아야 한다.
//
// 사용법:
//   node scripts/ingest-lending.mjs [파일경로]
...
const OUT = path.join(DATA, 'lending-balance.json');
...
fs.writeFileSync(OUT, JSON.stringify({
  meta: {
    source: `FREESIS 대차거래추이 (${path.basename(file)})`,
...
console.log(`\nlending-balance.json 생성: ${series.length}일  ${series[0].date}..${series.at(-1).date}`);
